...
def get_all_endpoints(server, token, scan_id):
    result_id = get_result_id_by_scan(server, token, scan_id)

    api = ApiGetCrawlData(url=server, api_key=token, scan_id=scan_id, result_id=result_id)
    json = do_request(api)

    loc_ids = [location["loc_id"] for location in json["locations"]]
    endpoints = [location["path"][1:] for location in json["locations"]]
    for loc_id in loc_ids:
        api_loc = ApiGetCrawlDataChildren(url=server, api_key=token, scan_id=scan_id, result_id=result_id, loc_id=loc_id)
...
def get_vulnerabilities(server, token, scan_id):
    SEVERITY = {"0": "Informational", "1": "Low", "2": "Medium", "3": "High", "4": "Critical"}
    result_id = get_result_id_by_scan(server, token, scan_id)

    api = ApiGetVulnerabilities(url=server, api_key=token, scan_id=scan_id, result_id=result_id)
...
def get_result_id_by_scan(server, token, scan_id):
    result_api = ApiGetResultByScan(url=server, api_key=token, scan_id=scan_id)
...
def fill_statistic(server, token, scan: dict, protocol: str, product: Product, address: str):
    ...
    print('saving')
    endpoints = get_all_endpoints(server, token, scan_id)
    endpoints_objs = []
    ...
    vulnerabilites = get_vulnerabilities(server, token, scan_id)
    ...
    while True:
        ...
        if status == "completed":
            ...
            if download_url:
                ...
                url = f"{server}/{correct_url}"

                response = requests.get(url, headers={"X-Auth": token}, stream=True, verify=False)
