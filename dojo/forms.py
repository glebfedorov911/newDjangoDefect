...
from dojo.endpoint.utils import (
...
class MultipleSelectWithPop(forms.SelectMultiple):
    def render(self, name, *args, **kwargs):
        ...
        popup_plus = (
            '<div class="input-group dojo-input-group">'
            + html
            + '<span class="input-group-btn"><a href="/'
            + name
            + '/add" class="btn btn-primary" class="add-another" id="add_id_'
            + name
            + '" onclick="return showAddAnotherPopup(this);"><span class="glyphicon glyphicon-plus"></span></a></span></div>'
        )
...
class Product_TypeForm(forms.ModelForm):
    description = forms.CharField(
        widget=forms.Textarea(attrs={}), required=False, label="Описание"
    )
...
class Edit_Product_Type_MemberForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["user"].queryset = Dojo_User.objects.order_by(
...
class Add_Product_Type_MemberForm(forms.ModelForm):
    users = forms.ModelMultipleChoiceField(
        queryset=Dojo_User.objects.none(), required=True, label="Пользователи"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["users"].queryset = (
            Dojo_User.objects.exclude(Q(is_superuser=True) | Q(id__in=current_members))
            .exclude(is_active=False)
            .order_by("first_name", "last_name")
        )
        self.fields["product_type"].disabled = True
        self.fields["role"].label = "Роль"
        self.fields["product_type"].label = "Тип продукта"
...
class Add_Product_Type_Member_UserForm(forms.ModelForm):
    product_types = forms.ModelMultipleChoiceField(
        queryset=Product_Type.objects.none(), required=True, label="Типы продукта"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["product_types"].queryset = get_authorized_product_types(
...
class ProductForm(forms.ModelForm):
    ...
    description = forms.CharField(
        widget=forms.Textarea(attrs={}), required=True, label="Описание"
    )

    prod_type = forms.ModelChoiceField(
        label="Тип продукта", queryset=Product_Type.objects.none(), required=True
    )

    sla_configuration = forms.ModelChoiceField(
        label="Конфигурация SLA",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["prod_type"].queryset = get_authorized_product_types(
        ...
        self.fields["enable_full_risk_acceptance"].label = (
            "Включить полное принятие риска"
        )
...
class EditFindingGroupForm(forms.ModelForm):
    name = forms.CharField(
        max_length=255, required=True, label="Нахождение названия группы"
    )
    jira_issue = forms.CharField(
        max_length=255,
        required=False,
        label="Связанный выпуск Jira",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["push_to_jira"].help_text = (
            "Проверка это будет перезаписать содержание вашей проблемы Jira или создаст ее."
        )
...
class Add_Product_Member_UserForm(forms.ModelForm):
    products = forms.ModelMultipleChoiceField(
        queryset=Product.objects.none(), required=True, label="Продукты"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["products"].queryset = get_authorized_products(
...
class NoteTypeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["is_single"].label = "Одиночный"
        self.fields["is_mandatory"].label = "обязательный"
...
class ImportScanForm(forms.Form):
    ...
    api_scan_configuration = forms.ModelChoiceField(
        ...
        required=False,
        label="Конфигурация сканирования API",
    )
    ...
    tags = TagField(
        required=False,
        help_text="Добавьте теги, которые помогают описать это сканирование."
    ...
    close_old_findings = forms.BooleanField(
        help_text="Старые выводы больше не присутствуют в новом отчете, как смягчаются при смягчении при импорте."
        "If service has been set, only the findings for this service will be closed. "
        "This only affects findings within the same engagement.",
        label="Близкие старые выводы в рамках этого задания ",
        required=False,
        initial=False,
    )
    ...
    if is_finding_groups_enabled():
        group_by = forms.ChoiceField(
            ...
            help_text="Выберите опцию для автоматической группировки новых результатов по выбранной опции.",
        )
...
class ReImportScanForm(forms.Form):
    ...
    tags = TagField(
        required=False,
        help_text="Измените существующие теги, которые помогают описать это сканирование."
        "Choose from the list or add new tags. Press Enter key to add.",
    )
    file = forms.FileField(
        widget=forms.widgets.FileInput(
    ...
    close_old_findings = forms.BooleanField(
        help_text="Выберите, если старые выводы больше не присутствуют в отчете, становятся закрытыми при смягчении при импорте.",
        required=False,
        initial=True,
    )
    version = forms.CharField(
        max_length=100,
        required=False,
        help_text="Версия, которая будет установлена на существующем испытательном объекте.Оставьте пустым, чтобы оставить существующую ценность на месте.",
    )
    branch_tag = forms.CharField(
        max_length=100,
        required=False,
    ...
    service = forms.CharField(
        max_length=200,
        required=False,
        help_text="Сервис-это автономная часть функциональности внутри продукта.Это необязательное поле, которое используется при дедуплировании выводов при установке.",
    )
    source_code_management_uri = forms.URLField(
    ...
    def clean(self):
        ...
        if requires_file(self.scan_type) and not file:
            raise forms.ValidationError(
                "Uploading a report file is required for re-uploading findings."
            )
...
class MergeFindings(forms.ModelForm):
    ...
    dynamic_raw = forms.BooleanField(
        label="Динамические запросы сканера сканера",
        initial=True,
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["finding_to_merge_into"] = forms.ModelChoiceField(
            ...
            initial=0,
            required="False",
            label="Поиск, чтобы слиться в",
            help_text="Выбранные ниже выводы будут объединены в этот вывод.",
        )
...
class EditRiskAcceptanceForm(forms.ModelForm):
    ...
    recommendation = forms.ChoiceField(
        ...
        label="Рекомендация безопасности",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["path"].help_text = (
            "Существующее доказательство загружено: %s" % self.instance.filename()
...
class AddFindingsRiskAcceptanceForm(forms.ModelForm):
    accepted_findings = forms.ModelMultipleChoiceField(
        ...
        label="Добавить результаты, как принятые:",
    )
...
class EngForm(forms.ModelForm):
    ...
    description = forms.CharField(
        ...
        required=False,
        help_text="Описание взаимодействия и деталей, касающихся взаимодействия.",
        label="Описание",
    )
    product = forms.ModelChoiceField(
        label="Продукт", queryset=Product.objects.none(), required=True
    )
    target_start = forms.DateField(
        widget=forms.TextInput(attrs={"class": "datepicker", "autocomplete": "off"}),
        label="Запланированный старт",
    )
    target_end = forms.DateField(
        widget=forms.TextInput(attrs={"class": "datepicker", "autocomplete": "off"}),
        label="Запланированный финал",
    )
    lead = forms.ModelChoiceField(queryset=None, required=True, label="Руководитель")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if product:
            self.fields["preset"] = forms.ModelChoiceField(
                help_text="Настройки и заметки для выполнения этого взаимодействия.",
                required=False,
...
class AddFindingForm(forms.ModelForm):
    ...
    endpoints = forms.ModelMultipleChoiceField(
        Endpoint.objects.none(), required=False, label="Системы / конечные точки"
    )
    endpoints_to_add = forms.CharField(
        max_length=5000,
        required=False,
    ...
    def clean(self):
        ...
        if (cleaned_data["active"] or cleaned_data["verified"]) and cleaned_data[
            "duplicate"
        ]:
            raise forms.ValidationError(
        ...
        if (
            cleaned_data["active"]
            and "risk_accepted" in cleaned_data
            and cleaned_data["risk_accepted"]
        ):
            raise forms.ValidationError("Active findings cannot " "be risk accepted.")
...
class FindingForm(forms.ModelForm):
    ...
    group = forms.ModelChoiceField(
        ...
        help_text="Группа поиска, к которой принадлежит этот вывод, оставьте пустым, чтобы удалить обнаружение из группы.Группы могут быть созданы только с помощью объема редактирования на данный момент.",
    )
    date = forms.DateField(
        required=True,
        widget=forms.TextInput(attrs={"class": "datepicker", "autocomplete": "off"}),
    ...
    mitigated = forms.DateField(
        required=False,
        help_text="Дата и время, когда недостаток был исправлен",
        widget=forms.TextInput(attrs={"class": "datepicker", "autocomplete": "off"}),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self.instance, "test") or (
            ...
            if self.instance.risk_accepted:
                self.fields["risk_accepted"].help_text = (
                    "Снимите, чтобы не принять риск.Используйте полное принятие риска из раскрывающегося меню, если вам нужны расширенные настройки, такие как дата истечения."
                )
            elif self.instance.test.engagement.product.enable_simple_risk_acceptance:
                self.fields["risk_accepted"].help_text = (
                    "Проверьте, чтобы принять риск.Используйте полное принятие риска из раскрывающегося меню, если вам нужны расширенные настройки, такие как дата истечения."
                )
        ...
        if self.instance.duplicate:
            self.fields["duplicate"].help_text = (
                "Оригинальный вывод, который здесь дублируется (Readonly).Используйте страницу «Посмотреть поиск» для управления дублирующими отношениями.Неужелительное дублирование здесь сбросит этот статус дубликата, но запустит логику дедупликации."
            )
        else:
            self.fields["duplicate"].help_text = (
                "Вы можете отмечать результаты как дубликаты только со страницы поиска View."
            )
...
class ApplyFindingTemplateForm(forms.Form):
    ...
    tags = TagField(
        required=False,
        help_text="Добавьте теги, которые помогают описать этот шаблон обнаружения.Выберите из списка или добавьте новые теги.Нажмите клавишу Enter, чтобы добавить.",
    ...
    def __init__(self, template=None, *args, **kwargs):
        ...
        self.fields["tags"].autocomplete_tags = (
            Finding.tags.tag_model.objects.all().order_by("name")
        )
...
class FindingTemplateForm(forms.ModelForm):
    apply_to_findings = forms.BooleanField(
        required=False,
        help_text="Примените шаблон ко всем выводам, которые соответствуют этой CWE.(Обновление будет перезаписать смягчение, воздействие и ссылки для любых активных, проверенных результатов.)",
    )
...
class AddEndpointForm(forms.Form):
    ...
    product = forms.CharField(
        ...
        help_text="Продукт, который должен быть эта конечная точка",
        label="Продукт" "associated with.",
    )
    tags = TagField(
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["product"] = forms.ModelChoiceField(
            queryset=get_authorized_products(Permissions.Endpoint_Add), label="Продукт"
        )
    ...
    def clean(self):
        ...
        if "endpoint" in cleaned_data and "product" in cleaned_data:
            ...
            raise forms.ValidationError(
                "Please enter a valid URL or IP address.", code="invalid"
            )
...
class TypedNoteForm(NoteForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["note_type"] = forms.ModelChoiceField(
            queryset=queryset, label="Примечание типа", required=True
        )
...
class CloseFindingForm(forms.ModelForm):
    ...
    false_p = forms.BooleanField(
        initial=False, required=False, label="Ложно положительный"
    )
    out_of_scope = forms.BooleanField(
        initial=False, required=False, label="Вне сферы действия"
    )
    ...
    def clean(self):
        ...
        if len(cleaned_data.get("reviewers", [])) == 0:
            raise ValidationError(
                "Please select at least one user from the reviewers list"
            )
        return cleaned_data
...
class WeeklyMetricsForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(6):
            ...
            wmf_options.append(
                (
                    end_of_period.strftime("%b %d %Y %H %M %S %Z"),
...
class SimpleMetricsForm(forms.Form):
    date = forms.DateField(label="", widget=MonthYearWidget())
...
class Add_Group_MemberForm(forms.ModelForm):
    users = forms.ModelMultipleChoiceField(
        queryset=Dojo_Group_Member.objects.none(), required=True, label="Пользователи"
    )

    def __init__(self, *args, **kwargs):
        super(Add_Group_MemberForm, self).__init__(*args, **kwargs)
        self.fields["group"].disabled = True
        current_members = Dojo_Group_Member.objects.filter(
...
class Add_Group_Member_UserForm(forms.ModelForm):
    groups = forms.ModelMultipleChoiceField(
        queryset=Dojo_Group.objects.none(), required=True, label="Группа"
    )

    def __init__(self, *args, **kwargs):
        super(Add_Group_Member_UserForm, self).__init__(*args, **kwargs)
        self.fields["user"].disabled = True
        current_groups = Dojo_Group_Member.objects.filter(
            user=self.initial["user"]
        ).values_list("group", flat=True)
        self.fields["groups"].queryset = Dojo_Group.objects.exclude(
            id__in=current_groups
        )
        self.fields["role"].queryset = get_group_member_roles()
...
class Add_Product_Group_GroupForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        current_members = Product_Group.objects.filter(
            group=self.initial["group"]
        ).values_list("product", flat=True)
        self.fields["products"].queryset = get_authorized_products(
            Permissions.Product_Member_Add_Owner
...
class Add_Product_Type_Group_GroupForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        current_members = Product_Type_Group.objects.filter(
            group=self.initial["group"]
        ).values_list("product_type", flat=True)
        self.fields["product_types"].queryset = get_authorized_product_types(
            Permissions.Product_Type_Member_Add_Owner
...
class DojoUserForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if not get_current_user().is_superuser and not get_system_setting(
...
class ProductCountsFormBase(forms.Form):
    month = forms.ChoiceField(
        choices=list(MONTHS.items()), required=True, error_messages={"required": "*"}
    )
    year = forms.ChoiceField(
        choices=get_years, required=True, error_messages={"required": "*"}
    )
...
class ProductTagCountsForm(ProductCountsFormBase):
    product_tag = forms.ModelChoiceField(
        required=True,
    ...
    def __init__(self, *args, **kwargs):
        ...
        tags_available_to_user = Product.tags.tag_model.objects.filter(
            product__in=prods
        )
...
class ReportOptionsForm(forms.Form):
    yes_no = (("0", "No"), ("1", "Yes"))
    include_finding_notes = forms.ChoiceField(
        choices=yes_no, label="Нахождение заметок"
    )
    include_finding_images = forms.ChoiceField(
        choices=yes_no, label="Поиск изображений"
    )
    include_executive_summary = forms.ChoiceField(
        choices=yes_no, label="Управляющее резюме"
    )
    include_table_of_contents = forms.ChoiceField(choices=yes_no, label="Оглавление")
    include_disclaimer = forms.ChoiceField(
        choices=yes_no, label="Отказ от ответственности"
    )
...
class CustomReportOptionsForm(forms.Form):
    ...
    include_finding_notes = forms.ChoiceField(
        required=False, choices=yes_no, label="Включать поисковые заметки"
    )
    include_finding_images = forms.ChoiceField(
        choices=yes_no, label="Поиск изображений"
    )
    report_type = forms.ChoiceField(
        choices=(("HTML", "HTML"), ("AsciiDoc", "AsciiDoc")), label="Тип отчёта"
    )
...
class ExpressGITHUBForm(forms.ModelForm):
    ...
    issue_key = forms.CharField(
        required=True,
        help_text="Действительный идентификатор проблемы требуется для сбора необходимой информации.",
    )
...
class ToolConfigForm(forms.ModelForm):
    tool_type = forms.ModelChoiceField(
        queryset=Tool_Type.objects.all(), label="Тип инструмента"
    )
    ssh = forms.CharField(
...
class ToolProductSettingsForm(forms.ModelForm):
    tool_configuration = forms.ModelChoiceField(
        queryset=Tool_Configuration.objects.all(), label="Конфигурация инструмента"
    )
...
class EngagementPresetsForm(forms.ModelForm):
    ...
    notes = forms.CharField(
        widget=forms.Textarea(attrs={}),
        required=False,
        help_text="Описание того, что необходимо протестировать или настраивать среда для тестирования",
    )

    scope = forms.CharField(
        widget=forms.Textarea(attrs={}),
        required=False,
        help_text="Объем тестирования вовлеченности, IPs/Resources/URL's)",
    )
...
class SystemSettingsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["enable_deduplication"].label = "Удалить дубликаты"
        self.fields["delete_duplicates"].label = "Удалить дубликаты"
        self.fields["max_dupes"].label = "Максимальное количество дубликатов"
        self.fields["email_from"].label = "email от"
        self.fields["enable_jira"].label = "Включить интеграцию JIRA"
        self.fields["enable_jira_web_hook"].label = "Включить JIRA web hook"
        self.fields["disable_jira_webhook_secret"].label = "Отключить секрет web hook"
        self.fields["jira_webhook_secret"].label = "Jira вэбхук токен"
        self.fields["jira_minimum_severity"].label = "Jira минимальная тяжесть"
        self.fields["jira_labels"].label = "Лейблы Jira"
        self.fields["enable_mail_notifications"].label = "Включить уведомления на почту"
        self.fields["mail_notifications_to"].label = "На почту"
        self.fields["false_positive_history"].label = "Ложноположительная история"
        self.fields["retroactive_false_positive_history"].label = (
            "Ретроактивная ложноположительная история"
        )
        self.fields["url_prefix"].label = "URL Префикс"
        self.fields["team_name"].label = "Имя команды"
        self.fields["time_zone"].label = "часовая зона"
        self.fields["default_group"].label = "Группа по умолчанию"
        self.fields["default_group_role"].label = "Роль в группе по умолчанию"
        self.fields["default_group_email_pattern"].label = "Паттерн email для группы"
        self.fields["risk_acceptance_form_default_days"].label = "Срок принятия рисков"
        self.fields["credentials"].label = "Квалификация"
...
class NotificationsForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["product_type_added"].label = "Добавлен тип продукта"
        self.fields["product_added"].label = "Добавлен продукт"
        self.fields["engagement_added"].label = "Задания добавлены"
        self.fields["test_added"].label = "Добавлен тест"
        self.fields["scan_added"].label = "Добавлено сканирование"
        self.fields["scan_added_empty"].label = "Добавлено пустое сканирование"
        self.fields["jira_update"].label = "Обновление JIRA"
        self.fields["upcoming_engagement"].label = "Предстоящее задание"
        self.fields["stale_engagement"].label = "Устаревшие задания"
        self.fields["auto_close_engagement"].label = "Автоматическое закрытие"
        self.fields["close_engagement"].label = "Закрытое взаимодействие"
        self.fields["user_mentioned"].label = "Пользователь упомянул"
        self.fields["code_review"].label = "Код ревью"
        self.fields["review_requested"].label = "Обзор запрошен"
        self.fields["other"].label = "Другое"
        self.fields["sla_breach_combined"].label = "Нарушение SLA (в совокупности)"
...
class ProductNotificationsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["scan_added"].label = "Сканирование добавлено"
        self.fields["engagement_added"].label = "Задание добавлено"
        self.fields["close_engagement"].label = "Задание закрыто"
        self.fields["test_added"].label = "Тест добавлен"
        self.fields["sla_breach_combined"].label = "Нарушение SLA (В совокупности)"
...
class CredUserForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(CredUserForm, self).__init__(*args, **kwargs)
        self.fields["name"].label = "Имя"
        self.fields["username"].label = "Имя пользователя"
        self.fields["password"].label = "Пароль"
        self.fields["role"].label = "Роль"
        self.fields["authentication"].label = "Аутентификация"
        self.fields["http_authentication"].label = "HTTP аутентификация"
        self.fields["description"].label = "Описание"
        self.fields["environment"].label = "Окружающая среда"
        self.fields["login_regex"].label = "Шаблонное выражение логина"
        self.fields["logout_regex"].label = "Шаблонное выражение выхода"
...
class GITHUB_Product_Form(forms.ModelForm):
    git_conf = forms.ModelChoiceField(
        queryset=GITHUB_Conf.objects.all(), label="Конфигурация GitHub", required=False
    )
...
class JIRAProjectForm(forms.ModelForm):
    inherit_from_product = forms.BooleanField(
        label="Унаследовать настройки JIRA от продукта", required=False
    )
    jira_instance = forms.ModelChoiceField(
        queryset=JIRA_Instance.objects.all(), label="Джира экземпляр", required=False
    )
    issue_template_dir = forms.ChoiceField(
        required=False,
        choices=JIRA_TEMPLATE_CHOICES,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.target == "engagement":
            ...
            self.fields["project_key"].widget = forms.TextInput(
                attrs={
                    "placeholder": "JIRA settings inherited from product "
                    "%s"
                    "" % product_name
                }
            )
            self.fields["project_key"].help_text = (
                "Настройки JIRA унаследованы от продукта"
                "%s"
                ", unless configured differently here." % product_name
            )
            self.fields["jira_instance"].help_text = (
                "Настройки JIRA унаследованы от продукта"
                "%s"
                " , unless configured differently here." % product_name
            )
            ...
            if self.instance.id:
                ...
                self.fields["inherit_from_product"].help_text = (
                    "После того, как участие сохранилось проект JIRA, вы не можете вернуться к наследству, чтобы избежать нарушения существующих проблем JIRA"
                )
                ...
                if (
                    ...
                    and not (self.prefix + "-jira_instance") in self.data
                ):
                    logger.debug("setting jira project fields from product2")
                    self.initial["jira_instance"] = (
                        jira_project_product.jira_instance.id
                        if jira_project_product.jira_instance
                        else None
                    )
                    self.initial["project_key"] = jira_project_product.project_key
                    self.initial["issue_template_dir"] = (
                        jira_project_product.issue_template_dir
                    )
                    self.initial["component"] = jira_project_product.component
                    self.initial["custom_fields"] = jira_project_product.custom_fields
                    self.initial["default_assignee"] = (
                        jira_project_product.default_assignee
                    )
                    self.initial["jira_labels"] = jira_project_product.jira_labels
                    self.initial["add_vulnerability_id_to_jira_label"] = (
                        jira_project_product.add_vulnerability_id_to_jira_label
                    )
                    self.initial["push_all_issues"] = (
                        jira_project_product.push_all_issues
                    )
                    self.initial["enable_engagement_epic_mapping"] = (
                        jira_project_product.enable_engagement_epic_mapping
                    )
                    self.initial["push_notes"] = jira_project_product.push_notes
                    self.initial["product_jira_sla_notification"] = (
                        jira_project_product.product_jira_sla_notification
                    )
                    self.initial["risk_acceptance_expiration_notification"] = (
                        jira_project_product.risk_acceptance_expiration_notification
                    )
                    ...
                    self.fields["risk_acceptance_expiration_notification"].disabled = (
                        True
                    )
    ...
    def clean(self):
        ...
        logger.debug(
            "clean: inherit: %s", self.cleaned_data.get("inherit_from_product", False)
        )
        if not self.cleaned_data.get("inherit_from_product", False):
            ...
            if self.target == "engagement":
                raise ValidationError(
                    "JIRA Project needs a JIRA Instance and JIRA Project Key, or choose to inherit settings from product"
                )
            else:
                raise ValidationError(
                    "JIRA Project needs a JIRA Instance and JIRA Project Key, leave empty to have no JIRA integration setup"
                )
...
class GITHUBFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["push_to_github"].help_text = (
            "Проверка этого будет перезаписать содержание вашей проблемы GitHub или создаст его."
        )
...
class JIRAFindingForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        if is_finding_groups_enabled():
            self.fields["push_to_jira"].help_text = (
                "Проверка это будет перезаписать содержание вашей проблемы Jira или создаст ее.Если этот вывод является частью группы поиска, группа будет выдвинуть вместо вывода."
            )
        else:
            self.fields["push_to_jira"].help_text = (
                "Проверка это будет перезаписать содержание вашей проблемы Jira или создаст ее."
        ...
        if (
            ...
            self.fields["jira_issue"].help_text = (
                "Изменение связанной проблемы Jira для поиска групп не поддерживается (пока)."
            )
...
class JIRAImportScanForm(forms.Form):
    ...
    push_to_jira = forms.BooleanField(
        required=False,
        label="Толк к Джире",
        help_text="Проверка этого создаст новую проблему Jira для каждого нового вывода.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            if self.instance.has_jira_issue:
                ...
                self.fields["push_to_jira"].help_text = (
                    "Проверка этого обновит существующий Epic в Jira."
                )

    push_to_jira = forms.BooleanField(
        required=False,
        label="Создать Epic",
        help_text="Проверка этого создаст эпос в Джире для этого задания.",
    )
    epic_name = forms.CharField(
        max_length=200,
        required=False,
        help_text="Эпическое имя в Джире.Если не указано, это по умолчанию на имя задания",
    )
    epic_priority = forms.CharField(
        max_length=200,
        required=False,
        help_text="Эпический приоритет.Если не указано, будет использоваться приоритет по умолчанию JIRA",
    )
...
class LoginBanner(forms.Form):
    ...
    banner_message = forms.CharField(required=False, label="Сообщение баннера")
...
class CreateQuestionForm(forms.Form):
    type = forms.ChoiceField(
        choices=(("---", "-----"), ("text", "Текст"), ("choice", "Выбор")), label="Тип"
    )
    ...
    optional = forms.BooleanField(
        help_text="Если выбрано, пользователь не делает должен ответить на этот вопрос",
        initial=False,
        required=False,
