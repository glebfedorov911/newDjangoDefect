...
# See https://documentation.synergyvms.com/getting_started/configuration/ for options
...
env = environ.FileAwareEnv(
    ...
    DD_DOCUMENTATION_URL=(str, "https://documentation.synergyvms.com"),
...
if len(env("DD_HASHCODE_FIELDS_PER_SCANNER")) > 0:
    ...
    for key, value in env_hashcode_fields_per_scanner.items():
        if key in HASHCODE_FIELDS_PER_SCANNER:
            logger.info(
                "Replacing {} with value {} (previously set to {}) from env var DD_HASHCODE_FIELDS_PER_SCANNER".format(
                    key, value, HASHCODE_FIELDS_PER_SCANNER[key]
                )
            )
            HASHCODE_FIELDS_PER_SCANNER[key] = value
        if key not in HASHCODE_FIELDS_PER_SCANNER:
            logger.info(
                "Adding {} with value {} from env var DD_HASHCODE_FIELDS_PER_SCANNER".format(
                    key, value
                )
            )
            HASHCODE_FIELDS_PER_SCANNER[key] = value
...
DEDUPLICATION_ALGORITHM_PER_PARSER = {
    ...
    "Indepo Scan": DEDUPE_ALGO_HASH_CODE,
...
if len(env("DD_DEDUPLICATION_ALGORITHM_PER_PARSER")) > 0:
    ...
    for key, value in env_dedup_algorithm_per_parser.items():
        if key in DEDUPLICATION_ALGORITHM_PER_PARSER:
            logger.info(
                "Replacing {} with value {} (previously set to {}) from env var DD_DEDUPLICATION_ALGORITHM_PER_PARSER".format(
                    key, value, DEDUPLICATION_ALGORITHM_PER_PARSER[key]
                )
            )
            DEDUPLICATION_ALGORITHM_PER_PARSER[key] = value
        if key not in DEDUPLICATION_ALGORITHM_PER_PARSER:
            logger.info(
                "Adding {} with value {} from env var DD_DEDUPLICATION_ALGORITHM_PER_PARSER".format(
                    key, value
                )
            )
            DEDUPLICATION_ALGORITHM_PER_PARSER[key] = value
...
warnings.filterwarnings(
    "ignore", message="polymorphic.base.ManagerInheritanceWarning.*"
)
