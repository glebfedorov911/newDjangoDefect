...
if [ "${GENERATE_TLS_CERTIFICATE}" = true ]; then
  openssl req  \
      ...
      -days 365 \
      -newkey rsa:4096 \
      -keyout /etc/angie/ssl/angie.key \
      -out /etc/angie/ssl/angie.crt \
      -subj "/C=DE/ST=City/L=City/O=Global Security/OU=IT Department/CN=angie"
fi

if [ "${USE_TLS}" = true ]; then
  NGINX_CONFIG="/etc/angie/angie_TLS.conf"
else
  NGINX_CONFIG="/etc/angie/angie.conf"
fi

if [ "${NGINX_METRICS_ENABLED}" = true ]; then
  sed -i "s/#stub_status/stub_status/g;" $NGINX_CONFIG
  echo "angie metrics are enabled"
fi

if [ "${METRICS_HTTP_AUTH_PASSWORD}" != "" ]; then
  sed -i "s/#auth_basic/auth_basic/g;" $NGINX_CONFIG
  rm -rf /etc/angie/.htpasswd
  openssl_passwd=$(openssl passwd -apr1 "$METRICS_HTTP_AUTH_PASSWORD")
  echo "$METRICS_HTTP_AUTH_USER":"$openssl_passwd" >> /etc/angie/.htpasswd
...
exec angie -c $NGINX_CONFIG -g "daemon off;"
